...
#ifndef ATHENA_BEADING_STRATEGY_H
#define ATHENA_BEADING_STRATEGY_H
...
public:
    ...
    BeadingStrategy(coord_t bead_spacing, coord_t extrusion_width, double wall_split_middle_threshold,
                    double wall_add_middle_threshold, coord_t default_transition_length,
    ...
    double getAddMiddleThreshold() const;
    double getTransitioningAngle() const;

    coord_t getBeadSpacing() const { return bead_spacing; }
    coord_t getExtrusionWidth() const { return extrusion_width; }

protected:
    std::string name;

    coord_t bead_spacing;    //! Spacing between bead centerlines (controls path placement)
    coord_t extrusion_width; //! Actual extrusion width (controls material output)

    double
        wall_split_middle_threshold; //! Threshold when a middle wall should be split into two, as a ratio of the bead spacing.

    double
        wall_add_middle_threshold; //! Threshold when a new middle wall should be added between an even number of walls, as a ratio of the bead spacing.
...
#endif // ATHENA_BEADING_STRATEGY_H
