...
#ifndef ATHENA_UTILS_POLYLINE_STITCHER_H
#define ATHENA_UTILS_POLYLINE_STITCHER_H
...
public:
                       ...
                       coord_t snap_distance = scaled<coord_t>(0.01))
    {
        ...
        SparsePointGrid<PathsPointIndex<Paths>, PathsPointIndexLocator<Paths>> grid(max_stitch_distance,
                                                                                    lines.size() * 2);
        ...
        for (size_t line_idx = 0; line_idx < lines.size(); line_idx++)
        {
            ...
            for (bool go_in_reverse_direction :
                 {false,
                  true}) // first go in the unreversed direction, to try to prevent the chain.reverse() operation.
                ...
                while (true)
                {
                    ...
                    grid.processNearby(
                        ...
                        std::function<bool(const PathsPointIndex<Paths> &)>(
                            [from, &chain, &closest, &closest_is_closing_polygon, &closest_distance, &processed,
                             &chain_length, go_in_reverse_direction, max_stitch_distance, snap_distance,
                             should_close](const PathsPointIndex<Paths> &nearby) -> bool
                            {
                                bool is_closing_segment = false;
                                coord_t dist =
                                    (nearby.p().template cast<int64_t>() - from.template cast<int64_t>()).norm();
                                if (dist > max_stitch_distance)
                                {
                                    return true; // keep looking
                                }
                                if ((nearby.p().template cast<int64_t>() -
                                     make_point(chain.front()).template cast<int64_t>())
                                        .squaredNorm() < snap_distance * snap_distance)
                                {
                                    if (chain_length + dist <
                                            3 * max_stitch_distance // prevent closing of small poly, cause it might be able to continue making a larger polyline
                                    ...
                                    {
                                        dist += scaled<coord_t>(
                                            0.01); // prefer continuing polyline over closing a polygon; avoids closed zigzags from being printed separately
                    ...
                    coord_t segment_dist = (make_point(chain.back()).template cast<int64_t>() -
                                            closest.p().template cast<int64_t>())
                                               .norm();
                    ...
                    {
                        chain_length += (make_point(chain[i]).template cast<int64_t>() -
                                         make_point(chain[i - 1]).template cast<int64_t>())
                                            .norm();
                    }
                    should_close =
                        should_close &
                        !isOdd(
                            (*closest.polygons)
                                [closest.poly_idx]); //If we connect an even to an odd line, we should no longer try to close it.
...
#endif // ATHENA_UTILS_POLYLINE_STITCHER_H///|/ Copyright (c) 2026 jbam
