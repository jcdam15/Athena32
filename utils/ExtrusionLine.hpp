...
#ifndef ATHENA_UTILS_EXTRUSION_LINE_H
#define ATHENA_UTILS_EXTRUSION_LINE_H
...
struct ExtrusionLine
{
    ...
    std::vector<ExtrusionJunction>::iterator insert(std::vector<ExtrusionJunction>::const_iterator pos, iterator first,
                                                    iterator last)
    {
     ...
     */
    void simplify(int64_t smallest_line_segment_squared, int64_t allowed_error_distance_squared,
...
static inline Slic3r::ThickPolyline to_thick_polyline(const Athena::ExtrusionLine &line_junctions)
{
...
static inline Slic3r::ThickPolyline to_thick_polyline(const ClipperZUtils::ZPath &path)
{
    assert(path.size() >= 2);
    Slic3r::ThickPolyline out;
...
using VariableWidthLines = std::vector<ExtrusionLine>; //<! Variable width extrusion lines for perimeter generation
