...
void ExtrusionLine::simplify(const int64_t smallest_line_segment_squared, const int64_t allowed_error_distance_squared,
                             const int64_t maximum_extrusion_area_deviation)
{
    ...
    int64_t accumulated_area_removed =
        int64_t(previous.p.x()) * int64_t(initial.p.y()) -
    ...
    for (size_t point_idx = 1; point_idx < junctions.size() - 1; point_idx++)
    {
        ...
        const int64_t removed_area_next =
            int64_t(current.p.x()) * int64_t(next.p.y()) -
            int64_t(current.p.y()) *
                int64_t(next.p.x()); // Twice the Shoelace formula for area of polygon per line segment.
        const int64_t negative_area_closing =
            int64_t(next.p.x()) * int64_t(previous.p.y()) -
            int64_t(next.p.y()) * int64_t(previous.p.x()); // Area between the origin and the short-cutting segment
        ...
        const auto height_2 = int64_t(double(area_removed_so_far) * double(area_removed_so_far) /
                                      double(base_length_2));
        ...
        if ((height_2 <= scaled<coord_t>(0.001) //Almost exactly colinear (barring rounding errors).
             ...
             Line::distance_to_infinite(current.p, previous.p, next.p) <=
                 scaled<double>(
                     0.001)) // Make sure that height_2 is not small because of cancellation of positive and negative areas
        ...
        if (length2 < smallest_line_segment_squared &&
            height_2 <=
                allowed_error_distance_squared) // Removing the junction (vertex) doesn't introduce too much error.
        {
            ...
            if (next_length2 > 4 * smallest_line_segment_squared)
            {
                ...
                if (!has_intersection ||
                    Line::distance_to_infinite_squared(intersection_point, previous.p, current.p) >
                        double(allowed_error_distance_squared) ||
                    (intersection_point - previous.p).cast<int64_t>().squaredNorm() >
                        smallest_line_segment_squared // The intersection point is way too far from the 'previous'
                    || (intersection_point - current.p).cast<int64_t>().squaredNorm() >
                           smallest_line_segment_squared) // and 'current' point, so it shouldn't replace 'current'
                {
                ...
                else
                {
                    ...
                    const ExtrusionJunction new_to_add = ExtrusionJunction(intersection_point, current.w,
                    ...
                    previous =
                        new_to_add; // Note that "previous" is only updated if we don't remove the junction (vertex).
...
int64_t ExtrusionLine::calculateExtrusionAreaDeviationError(ExtrusionJunction A, ExtrusionJunction B,
                                                            ExtrusionJunction C)
{
    /*
     * A             B                          C              A                                        C
    ...
    if (const coord_t width_diff = std::max(std::abs(B.w - A.w), std::abs(C.w - B.w)); width_diff > 1)
    {
        ...
        const int64_t weighted_average_width = (ab_length * ab_weight + bc_length * bc_weight) /
                                               (ab_length + bc_length);
...
bool ExtrusionLine::is_contour() const
{
    ...
    // Contours are clockwise, holes are counterclockwise
...
void extrusion_paths_append(ExtrusionPaths &dst, const ClipperZUtils::ZPaths &extrusion_paths, const ExtrusionRole role,
                            const Flow &flow, const uint32_t perimeter_index)
{
    for (const ClipperZUtils::ZPath &extrusion_path : extrusion_paths)
    {
        ThickPolyline thick_polyline = to_thick_polyline(extrusion_path);
        Slic3r::append(dst,
                       PerimeterGenerator::thick_polyline_to_multi_path(thick_polyline, role, flow, scaled<float>(0.05),
                                                                        float(SCALED_EPSILON), perimeter_index)
                           .paths);
    }
}

void extrusion_paths_append(ExtrusionPaths &dst, const ExtrusionLine &extrusion, const ExtrusionRole role,
                            const Flow &flow, const uint32_t perimeter_index)
{
    ThickPolyline thick_polyline = to_thick_polyline(extrusion);
    Slic3r::append(dst,
                   PerimeterGenerator::thick_polyline_to_multi_path(thick_polyline, role, flow, scaled<float>(0.05),
                                                                    float(SCALED_EPSILON), perimeter_index)
                       .paths);
