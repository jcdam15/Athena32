...
#ifndef ATHENA_UTILS_POLYGONS_POINT_INDEX_H
#define ATHENA_UTILS_POLYGONS_POINT_INDEX_H
...
public:
    ...
    bool operator<(const PathsPointIndex &other) const { return this->p() < other.p(); }
...
{
    Point operator()(const PathsPointIndex<Paths> &val) const { return make_point(val.p()); }
};
 ...
 */
template<>
struct hash<Slic3r::Athena::PolygonsPointIndex>
{
    size_t operator()(const Slic3r::Athena::PolygonsPointIndex &lpi) const { return Slic3r::PointHash{}(lpi.p()); }
};
} //namespace std
