...
#ifndef ATHENA_SKELETAL_TRAPEZOIDATION_H
#define ATHENA_SKELETAL_TRAPEZOIDATION_H
...
class SkeletalTrapezoidation
{
    ...
    double
        transitioning_angle; //!< How pointy a region should be before we apply the method. Equals 180* - limit_bisector_angle
    coord_t
        discretization_step_size; //!< approximate size of segments when parabolic VD edges get discretized (and vertex-vertex edges)
    ...
    static constexpr coord_t snap_dist = scaled<coord_t>(
        0.02); //!< Generic arithmatic inaccuracy. Only used to determine whether a transition really needs to insert an extra edge.
...
protected:
     ...
     */
    void transferEdge(const Point &from, const Point &to, const VD::edge_type &vd_edge, edge_t *&prev_edge,
                      const Point &start_source_point, const Point &end_source_point,
     ...
     */
    std::list<TransitionMidRef> dissolveNearbyTransitions(edge_t *edge_to_start, TransitionMiddle &origin_transition,
     ...
     */
    bool filterEndOfCentralTransition(edge_t *edge_to_start, coord_t traveled_dist, coord_t max_dist,
     ...
     * endpoints to.
     */
    void generateTransitionEnds(edge_t &edge, coord_t mid_R, coord_t transition_lower_bead_count,
     ...
     */
    bool generateTransitionEnd(edge_t &edge, coord_t start_pos, coord_t end_pos, coord_t transition_half_length,
                               double start_rest, double end_rest, coord_t transition_lower_bead_count,
                               ptr_vector_t<std::list<TransitionEnd>> &edge_transition_ends);
     ...
     * up.
     */
    bool isGoingDown(edge_t *outgoing, coord_t traveled_dist, coord_t transition_half_length,
    ...
    /*!
     * Apply bead width adjustments to fill remaining space in a beading.
     * This fixes center gaps by either expanding innermost beads (small gaps)
     * or creating/shrinking a variable-width center bead (larger gaps).
     * \param beading The beading to adjust.
     */
    void applyBeadWidthAdjustments(Beading &beading);
     ...
     */
    Beading interpolate(const Beading &left, double ratio_left_to_whole, const Beading &right,
     ...
     */
    std::shared_ptr<BeadingPropagation> getOrCreateBeading(node_t *node,
                                                           ptr_vector_t<BeadingPropagation> &node_beadings);
     ...
     */
    void generateJunctions(ptr_vector_t<BeadingPropagation> &node_beadings,
...
#endif // ATHENA_VORONOI_QUADRILATERALIZATION_H
