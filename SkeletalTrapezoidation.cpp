...
///|/ Copyright (c) 2026 jbam

using coord64_t = int64_t;
...
// Athena: Where Arachne's variable threads meet their match with precise, fixed-width execution
#include "utils/PolygonsPointIndex.hpp"
#include "libslic3r/Geometry/VoronoiUtils.hpp"
#include "ankerl/unordered_dense.h"
#include "libslic3r/Athena/SkeletalTrapezoidationEdge.hpp"
#include "libslic3r/Athena/SkeletalTrapezoidationJoint.hpp"
...
#define SKELETAL_TRAPEZOIDATION_BEAD_SEARCH_MAX \
    1000 //A limit to how long it'll keep searching for adjacent beads. Increasing will re-use beadings more often (saving performance), but search longer for beading (costing performance).

namespace Slic3r::Athena
{

#ifdef ATHENA_DEBUG
static void export_graph_to_svg(const std::string &path, SkeletalTrapezoidationGraph &graph, const Polygons &polys,
                                const std::vector<std::shared_ptr<LineJunctions>> &edge_junctions = {},
                                const bool beat_count = true, const bool transition_middles = true,
                                const bool transition_ends = true)
{
    const std::vector<std::string> colors = {"blue", "cyan",   "red",   "orange", "magenta",
    ...
    for (const auto &edge : graph.edges)
        svg.draw(Line(edge.from->p, edge.to->p), (edge.data.centralIsSet() && edge.data.isCentral()) ? "blue" : "cyan",
                 stroke_width);
    ...
    if (transition_middles)
    {
        ...
        {
            if (std::shared_ptr<std::list<SkeletalTrapezoidationEdge::TransitionMiddle>> transitions =
                    edge.data.getTransitions();
                transitions)
            {
                for (auto &transition : *transitions)
                {
                    ...
                    Point pt =
                        edge_line.a +
                        (edge_line.vector().cast<double>() * (double(transition.pos) / edge_length)).cast<coord_t>();
    ...
    if (transition_ends)
    {
        for (auto &edge : graph.edges)
        {
            if (std::shared_ptr<std::list<SkeletalTrapezoidationEdge::TransitionEnd>> transitions =
                    edge.data.getTransitionEnds();
                transitions)
            {
                for (auto &transition : *transitions)
                {
...
void SkeletalTrapezoidation::transferEdge(const Point &from, const Point &to, const VD::edge_type &vd_edge,
                                          edge_t *&prev_edge, const Point &start_source_point,
                                          const Point &end_source_point, const std::vector<Segment> &segments)
{
...
Points SkeletalTrapezoidation::discretize(const VD::edge_type &vd_edge, const std::vector<Segment> &segments)
{
    ...
    {
        Point p = Geometry::VoronoiUtils::get_source_point(*(point_left ? left_cell : right_cell), segments.begin(),
                                                           segments.end());
        const Segment &s = Geometry::VoronoiUtils::get_source_segment(*(point_left ? right_cell : left_cell),
                                                                      segments.begin(), segments.end());
        return Geometry::VoronoiUtils::discretize_parabola(p, s, start, end, discretization_step_size,
                                                           transitioning_angle);
    }
    else //This is a straight edge between two points.
    {
        ...
        const auto projected_x = [x_axis_dir, x_axis_length, middle](Point from) //Project a point on the edge.
        {
            ...
            assert((vec.cast<int64_t>().dot(x_axis_dir.cast<int64_t>()) / int64_t(x_axis_length)) <=
        ...
        assert((middle.cast<int64_t>() + x_axis_dir.cast<int64_t>() * marking_end_x / int64_t(x_axis_length)).y() <=
               std::numeric_limits<coord_t>::max());
        Point marking_start = middle +
                              (x_axis_dir.cast<int64_t>() * marking_start_x / int64_t(x_axis_length)).cast<coord_t>();
        Point marking_end = middle +
                            (x_axis_dir.cast<int64_t>() * marking_end_x / int64_t(x_axis_length)).cast<coord_t>();
        int64_t direction = 1;
        ...
        for (coord_t step = 1; step < step_count; step++)
        {
            ...
            coord_t x_here = projected_x(
                here); //If we've surpassed the position of the extra markings, we may need to insert them first.
...
void SkeletalTrapezoidation::generateToolpaths(std::vector<VariableWidthLines> &generated_toolpaths,
                                               bool filter_outermost_central_edges)
{
#ifdef ATHENA_DEBUG
    static int iRun = 0;
#endif
...
void SkeletalTrapezoidation::updateIsCentral()
{
    ...
    for (edge_t &edge : graph.edges)
    {
        ...
        else if (std::max(edge.from->data.distance_to_boundary, edge.to->data.distance_to_boundary) <
...
bool SkeletalTrapezoidation::filterCentral(edge_t *starting_edge, coord_t traveled_dist, coord_t max_length)
{
    ...
    for (edge_t *next_edge = starting_edge->next; next_edge && next_edge != starting_edge->twin;
    ...
    for (node_t &node : graph.nodes)
    {
        if (node.isLocalMaximum())
        {
            if (node.data.distance_to_boundary < 0)
            {
                ...
                do
                {
                    node.data.distance_to_boundary =
                        std::min(node.data.distance_to_boundary,
...
bool SkeletalTrapezoidation::filterNoncentralRegions(edge_t *to_edge, coord_t bead_count, coord_t traveled_dist,
                                                     coord_t max_dist)
{
    ...
    {
        if (next_edge->to->data.distance_to_boundary >= r ||
            shorter_then(next_edge->to->p - next_edge->from->p, scaled<coord_t>(0.01)))
        {
    ...
    if (dissolve)
    {
        ...
        next_edge->to->data.bead_count = beading_strategy.getOptimalBeadCount(next_edge->to->data.distance_to_boundary *
                                                                              2);
...
void SkeletalTrapezoidation::generateTransitionMids(ptr_vector_t<std::list<TransitionMiddle>> &edge_transitions)
{
    for (edge_t &edge : graph.edges)
    {
        ...
        { // No transitions occur when both end points have the same distance_to_boundary
            ...
            if (edge.from->data.bead_count != edge.to->data.bead_count)
            {
                BOOST_LOG_TRIVIAL(warning)
                    << "Bead count " << edge.from->data.bead_count << " is different from " << edge.to->data.bead_count
                    << " even though distance to boundary is the same.";
            }
            continue;
        }
        ...
        {
            BOOST_LOG_TRIVIAL(warning)
                << "Transitioning the wrong way around! This function expects to transition from small R to big R, but was transitioning from "
                << start_R << " to " << end_R;
        }
...
void SkeletalTrapezoidation::filterTransitionMids()
{
    for (edge_t &edge : graph.edges)
    {
        ...
        {
            dissolveBeadCountRegion(&edge, transitions.back().lower_bead_count + 1,
                                    transitions.back().lower_bead_count);
...

        {
            ...
            should_dissolve_back |= filterEndOfCentralTransition(&edge, ab_size - transitions.back().pos,
                                                                 upper_transition_half_length, trans_bead_count);
        }
        ...
        std::list<TransitionMidRef> to_be_dissolved_front = dissolveNearbyTransitions(edge.twin, transitions.front(),
                                                                                      transitions.front().pos,
                                                                                      transition_filter_dist, going_up);
        bool should_dissolve_front = !to_be_dissolved_front.empty();
        for (TransitionMidRef &ref : to_be_dissolved_front)
        {
            dissolveBeadCountRegion(edge.twin, transitions.front().lower_bead_count,
                                    transitions.front().lower_bead_count + 1);
            ref.edge->data.getTransitions()->erase(ref.transition_it);
        }

        {
            ...
            coord_t lower_transition_half_length = beading_strategy.getTransitionAnchorPos(trans_bead_count) *
                                                   beading_strategy.getTransitioningLength(trans_bead_count);
            should_dissolve_front |= filterEndOfCentralTransition(edge.twin, transitions.front().pos,
                                                                  lower_transition_half_length, trans_bead_count + 1);
        }
    ...
    edge_t *edge_to_start, TransitionMiddle &origin_transition, coord_t traveled_dist, coord_t max_dist, bool going_up)
{
    ...
    for (edge_t *edge = edge_to_start->next; edge && edge != edge_to_start->twin; edge = edge->twin->next)
    {
        ...
        const coord_t width_deviation =
            std::abs(origin_radius - radius_here) *
        ...
        const coord_t line_width_deviation =
            dissolve_result_is_odd
                ? width_deviation
                : width_deviation /
                      2; // assume the deviation will be split over either 1 or 2 lines, i.e. assume wall_distribution_count = 1
        ...
        if (should_dissolve && !seen_transition_on_this_edge)
        {
            ...
            to_be_dissolved.splice(to_be_dissolved.end(),
                                   to_be_dissolved_here); // Transfer to_be_dissolved_here into to_be_dissolved
...
void SkeletalTrapezoidation::dissolveBeadCountRegion(edge_t *edge_to_start, coord_t from_bead_count,
...
bool SkeletalTrapezoidation::filterEndOfCentralTransition(edge_t *edge_to_start, coord_t traveled_dist,
                                                          coord_t max_dist, coord_t replacing_bead_count)
{
    if (traveled_dist > max_dist)
    {
        return false;
    ...
    for (edge_t *next_edge = edge_to_start->next; next_edge && next_edge != edge_to_start->twin;
         next_edge = next_edge->twin->next)
    {
        if (next_edge->data.isCentral())
        {
            coord_t length = (next_edge->to->p - next_edge->from->p).cast<int64_t>().norm();
            should_dissolve |= filterEndOfCentralTransition(next_edge, traveled_dist + length, max_dist,
...
void SkeletalTrapezoidation::generateAllTransitionEnds(ptr_vector_t<std::list<TransitionEnd>> &edge_transition_ends)
{
    for (edge_t &edge : graph.edges)
    {
        ...
        for (TransitionMiddle &transition_middle : transition_positions)
        {
            ...
            generateTransitionEnds(edge, transition_middle.pos, transition_middle.lower_bead_count,
                                   edge_transition_ends);
        }
    }
}

void SkeletalTrapezoidation::generateTransitionEnds(edge_t &edge, coord_t mid_pos, coord_t lower_bead_count,
                                                    ptr_vector_t<std::list<TransitionEnd>> &edge_transition_ends)
{
    ...
    { // Lower bead count transition end
        ...
        generateTransitionEnd(*edge.twin, start_pos, end_pos, transition_half_length, mid_rest, start_rest,
...
#ifdef DEBUG
        if (!generateTransitionEnd(edge, start_pos, end_pos, transition_half_length, mid_rest, end_rest,
                                   lower_bead_count, edge_transition_ends))
        {
            BOOST_LOG_TRIVIAL(warning)
...
#else
        generateTransitionEnd(edge, start_pos, end_pos, transition_half_length, mid_rest, end_rest, lower_bead_count,
                              edge_transition_ends);
#endif
    }
}

bool SkeletalTrapezoidation::generateTransitionEnd(edge_t &edge, coord_t start_pos, coord_t end_pos,
                                                   coord_t transition_half_length, double start_rest, double end_rest,
    ...
    { // Recurse on all further edges
        ...
        for (edge_t *outgoing = edge.next; outgoing && outgoing != edge.twin;)
        {
            ...
            if (central_edge_count > 1 && going_up &&
                isGoingDown(outgoing, 0, end_pos - ab_size + transition_half_length, lower_bead_count))
            ...
            bool is_going_down = generateTransitionEnd(*outgoing, 0, end_pos - ab_size, transition_half_length, rest,
                                                       end_rest, lower_bead_count, edge_transition_ends);
...
bool SkeletalTrapezoidation::isGoingDown(edge_t *outgoing, coord_t traveled_dist, coord_t max_dist,
                                         coord_t lower_bead_count) const
{
    ...
    {
        assert(upward_edge->data.hasTransitions() &&
    ...
    if (upward_edge->data.hasTransitions())
    {
        ...
        if (mid.lower_bead_count == lower_bead_count && ((is_upward && mid.pos + traveled_dist < max_dist) ||
...
void SkeletalTrapezoidation::applyBeadWidthAdjustments(Beading &beading)
{
    ...
    // BeadingStrategy computes based on spacing (distance between toolpaths)
    // Bead width adjustment must use spacing for calculations to account for overlap correctly
    coord_t nominal_spacing = beading_strategy.getBeadSpacing();
    coord_t nominal_width = beading_strategy.getExtrusionWidth();
    coord_t overlap = nominal_width - nominal_spacing; // Overlap amount per bead
    ...
    if (beading.bead_widths.size() >= 2 && beading.bead_widths.size() % 2 == 0 && beading.left_over != 0)
    {
        ...
        if (beading.left_over > 0)
        {
            ...
            // Apply adjustment: additional_width = gap_per_bead × adjustment_factor
            ...
            // Target spacing between bead centers: spacing = width × (1 - overlap%)
        ...
        else
        {
            ...
            // Apply adjustment: width_reduction = reduction_per_bead × adjustment_factor
    ...
    else if (beading.bead_widths.size() % 2 == 1)
    {
        ...
        // STAGE 2A: Positive left_over (gap exists, EXPAND center bead)
        if (beading.left_over > 0)
        {
            // Apply adjustment: width_change = left_over * adjustment_factor
        ...
        // STAGE 2B: Negative left_over (overfilled, CONTRACT center bead)
        else
        {
            // Apply adjustment: width_reduction = abs(left_over) * adjustment_factor
        ...
        if (!split_center_into_loop)
        {
            ...
            if (new_center_width > 0)
            {
                ...
                // would be at full width, but now it's adjusted, so the boundary position changes.
...
void SkeletalTrapezoidation::generateSegments()
{
    ...
    std::sort(
        upward_quad_mids.begin(), upward_quad_mids.end(),
        [](edge_t *a, edge_t *b)
        {
            ...
            { // Ordering between two 'upward' edges of the same distance is important when one of the edges is flat and connected to the other
                    ...
                    b->from->data.distance_to_boundary == b->to->data.distance_to_boundary)
                {
                    ...
                    coord_t a_dist_from_up = std::min(a->distToGoUp().value_or(max),
                                                      a->twin->distToGoUp().value_or(max)) -
                                             (a->to->p - a->from->p).cast<int64_t>().norm();
                    coord_t b_dist_from_up = std::min(b->distToGoUp().value_or(max),
                                                      b->twin->distToGoUp().value_or(max)) -
    ...
    { // Store beading
        for (node_t &node : graph.nodes)
        {
            ...
            {
                node_beadings.emplace_back(new BeadingPropagation(
                    beading_strategy.compute(node.data.distance_to_boundary * 2, node.data.bead_count)));
                node.data.setBeading(node_beadings.back());

                applyBeadWidthAdjustments(node_beadings.back()->beading);
                ...
                {
                    BOOST_LOG_TRIVIAL(warning)
                        << "If transitioning to an endpoint (ratio 0), the node should be exactly in the middle.";
                }
            }
            else
            {
                Beading low_count_beading = beading_strategy.compute(node.data.distance_to_boundary * 2,
                                                                     node.data.bead_count);
                Beading high_count_beading = beading_strategy.compute(node.data.distance_to_boundary * 2,
                                                                      node.data.bead_count + 1);
...
#ifdef ATHENA_DEBUG
    static int iRun = 0;
    export_graph_to_svg(debug_out_path("ST-generateSegments-before-propagation-%d.svg", iRun), this->graph,
                        this->outline);
#endif
...
#ifdef ATHENA_DEBUG
    export_graph_to_svg(debug_out_path("ST-generateSegments-upward-propagation-%d.svg", iRun), this->graph,
                        this->outline);
#endif
...
#ifdef ATHENA_DEBUG
    export_graph_to_svg(debug_out_path("ST-generateSegments-junctions-%d.svg", iRun), this->graph, this->outline,
                        edge_junctions);
#endif
...
SkeletalTrapezoidation::edge_t *SkeletalTrapezoidation::getQuadMaxRedgeTo(edge_t *quad_start_edge)
{
    ...
    if (!ret->next &&
        ret->to->data.distance_to_boundary - scaled<coord_t>(0.005) < ret->from->data.distance_to_boundary)
    {
        ret = ret->prev;
    }
    assert(ret);
...
void SkeletalTrapezoidation::propagateBeadingsUpward(std::vector<edge_t *> &upward_quad_mids,
                                                     ptr_vector_t<BeadingPropagation> &node_beadings)
{
    for (auto upward_quad_mids_it = upward_quad_mids.rbegin(); upward_quad_mids_it != upward_quad_mids.rend();
         ++upward_quad_mids_it)
    {
        ...
        assert((upward_edge->from->data.distance_to_boundary != upward_edge->to->data.distance_to_boundary ||
                shorter_then(upward_edge->to->p - upward_edge->from->p, central_filter_dist)) &&
...
void SkeletalTrapezoidation::propagateBeadingsDownward(edge_t *edge_to_peak,
                                                       ptr_vector_t<BeadingPropagation> &node_beadings)
{
...
SkeletalTrapezoidation::Beading SkeletalTrapezoidation::interpolate(const Beading &left, double ratio_left_to_whole,
                                                                    const Beading &right,
                                                                    coord_t switching_radius) const
{
    ...
    { // One inset disappeared between left and the merged one
        ...
        float new_ratio = static_cast<float>(switching_radius - right.toolpath_locations[next_inset_idx]) /
                          static_cast<float>(left.toolpath_locations[next_inset_idx] -
                                             right.toolpath_locations[next_inset_idx]);
...
void SkeletalTrapezoidation::generateJunctions(ptr_vector_t<BeadingPropagation> &node_beadings,
                                               ptr_vector_t<LineJunctions> &edge_junctions)
{
    for (edge_t &edge_ : graph.edges)
    {
        ...
        size_t start_junction_idx = (std::max(size_t(1), beading->toolpath_locations.size()) - 1) / 2;
        for (junction_idx = start_junction_idx; junction_idx < num_junctions; junction_idx--)
        {
        ...
        for (; junction_idx < num_junctions;
             junction_idx--) //When junction_idx underflows, it'll be more than num_junctions too.
        {
            coord_t bead_R = beading->toolpath_locations[junction_idx];
            assert(bead_R >= 0);
    ...
    node_t *node, ptr_vector_t<BeadingPropagation> &node_beadings)
{
    if (!node->data.hasBeading())
    {
        ...
        { // This bug is due to too small central edges
            ...
            for (edge_t *edge = node->incident_edge; edge && (first || edge != node->incident_edge);
                 edge = edge->twin->next)
            {
                if (edge->data.isCentral())
                {
                ...
                dist = std::min(dist, edge->to->data.distance_to_boundary +
                                          coord_t((edge->to->p - edge->from->p).cast<int64_t>().norm()));
                first = false;
            }
            if (!has_central_edge)
            {
    ...
    auto result = node->data.getBeading();
    return result;
}

// Note: If crash occurs after "returning from getOrCreateBeading()", the issue is in the CALLER

std::shared_ptr<SkeletalTrapezoidationJoint::BeadingPropagation> SkeletalTrapezoidation::getNearestBeading(
    node_t *node, coord_t max_dist)
{
    struct DistEdge
    {
        edge_t *edge_to;
    ...
    bool first = true;
    for (edge_t *outgoing = node->incident_edge; outgoing && (first || outgoing != node->incident_edge);
    ...
    { // Prevent endless recursion
        ...
        { // recurse
            ...
            {
                further_edges.emplace(further_edge,
                                      here.dist + (further_edge->to->p - further_edge->from->p).cast<int64_t>().norm());
            }
        }
    }
    return nullptr;
                                                ...
                                                bool force_new_path, bool from_is_3way, bool to_is_3way)
{
    ...
    if (generated_toolpaths[inset_idx].empty() || generated_toolpaths[inset_idx].back().is_odd != is_odd ||
        generated_toolpaths[inset_idx].back().junctions.back().perimeter_index !=
            inset_idx // inset_idx should always be consistent
    )
    {
        force_new_path = true;
    }
...
void SkeletalTrapezoidation::connectJunctions(ptr_vector_t<LineJunctions> &edge_junctions)
{
    ...
    while (!unprocessed_quad_starts.empty())
    {
        ...
        do
        {
            ...
            assert(std::abs(int(from_junctions.size()) - int(to_junctions.size())) <=
                   1); // at transitions one end has more beads
            ...
            for (size_t junction_rev_idx = 0; junction_rev_idx < segment_count; junction_rev_idx++)
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(warning) << "Connecting two perimeters with different indices! Perimeter "
                                               << from.perimeter_index << " and " << to.perimeter_index;
                }
