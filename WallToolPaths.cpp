...
WallToolPaths::WallToolPaths(const Polygons &outline, const coord_t bead_width_0, const coord_t bead_width_x,
                             const size_t inset_count, const coord_t wall_0_inset, const coordf_t layer_height,
                             const WallToolPathsParams &params, int layer_id)
    : outline(outline)
    , bead_width_0(bead_width_0)
    , bead_width_x(bead_width_x)
    , inset_count(inset_count)
    ...
    , small_area_length(static_cast<double>(bead_width_0) / 2.)
    , wall_transition_filter_deviation(scaled<coord_t>(0.025)) // Default value
    , wall_transition_length(params.wall_transition_length)
    , toolpaths_generated(false)
    , print_object_config(nullptr) // No config for OrcaSlicer compatibility
    , fixed_width_external(0)
    , fixed_width_internal(0)
    , spacing_override_external(0)
    , spacing_override_internal(0)
    ...
    , m_original_inset_count(inset_count)
{
    this->min_nozzle_diameter = 0.4f; // Default nozzle diameter
}
...
const std::vector<VariableWidthLines> &WallToolPaths::generate()
{
    ...
    const double transitioning_angle = Geometry::deg2rad(this->wall_transition_length / 10.0); // Simplified
    ...
    else
    {
        ...
        (void) this->wall_transition_length;
